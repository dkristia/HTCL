...
const specialIdentifiers = ['let', 'const', 'args', 'arg', 'return', 'counter', 'type', 'while', 'for', 'if', 'elseIf', 'else', 'from', 'to', 'condition'];

const isAlpha = (c: string | undefined): boolean => {
    return c !== undefined && (c.toUpperCase() !== c.toLowerCase() || (c === '_' || c === '-'));
};
        ...
        if (src[0] === '/' && src[1] === '/') {
            ...
            if (isNumeric(src[0]) || isAlpha(src[0])) {
                ...
                if (!isNaN(Number(word))) {
                    ...
                    tokens.push({ value: word, type: TokenType[word.charAt(0).toUpperCase() + word.slice(1) as keyof typeof TokenType] });
                } else {
